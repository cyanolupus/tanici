...
fn create_user(csv: String) -> User {
    ...
    let units_cl: Vec<Unit> = units.clone();
...
fn check(user: User) -> u8 {
    ...
    let mut units = check_req(user.units, a_req, "専門    科目");

    let mut countn0: f32 = 0.0;
    let mut countn: f32 = 0.0;
    while units.iter().any(|x| x.unit_group == "A" && x.grade_num > -2.0) {
        let unitsbkt2: Vec<Unit> = units.clone();
        let unit = unitsbkt2.iter().find(|x| x.unit_group == "A" && x.grade_num > -2.0).unwrap();
        println!("専門    科目: {:>2.1} {} {}", unit.unit_num, unit.unit_id, unit.unit_name);
    ...
    units = check_req(units, b_req, "専門基礎科目");
    let mut misc: f32 = 0.0;
    let mut cseng: f32 = 0.0;
    let mut gb1: f32 = 0.0;
    let mut ga1: f32 = 0.0;
    while units.iter().any(|x| x.unit_group == "B" && x.grade_num > -2.0) {
        let unitsbkt2: Vec<Unit> = units.clone();
        let unit = unitsbkt2.iter().find(|x| x.unit_group == "B" && x.grade_num > -2.0).unwrap();
    ...
    units = check_req(units, c_req, "共通基礎科目");
    ...
    while units.iter().any(|x| x.unit_group == "C" && x.grade_num > -2.0) {
        let unitsbkt2: Vec<Unit> = units.clone();
        let unit = unitsbkt2.iter().find(|x| x.unit_group == "C" && x.grade_num > -2.0).unwrap();
    ...
    while units.iter().any(|x| x.unit_group == "C0" && x.grade_num > -2.0) {
        let unitsbkt2: Vec<Unit> = units.clone();
        let unit = unitsbkt2.iter().find(|x| x.unit_group == "C0" && x.grade_num > -2.0).unwrap();
